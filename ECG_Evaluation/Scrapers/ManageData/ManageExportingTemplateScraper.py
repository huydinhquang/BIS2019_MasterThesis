...
def update_exp_template(exp_template_col, field_name, item_id, list_new_values: ExportingTemplate):
    ...
    x = exp_template_col.update_one(query, new_values)
    return x.modified_count

def delete_exp_template(exp_template_col, field_name, item_id):
    query = {field_name: item_id}
    x = exp_template_col.delete_one(query)
